...
# Import Neo4j optimization
import sys
from pathlib import Path
# Add parent directory to import neo4j_native_search
...
def sidebar():
    ...
    with st.sidebar:
        ...
        if check_building_status(Build_Status):
            # Neo4j Settings Toggle
            use_neo4j = st.toggle("Use Neo4j Optimization", value=True, 
                                 help="Enable Neo4j-native search (faster, lower memory)")
            
            Enable_Search = st.toggle("Search Engine", value=True)
            
            if Enable_Search and not st.session_state.settings.get('engine_running'):
                config_obj = NGConfig(all_config())
                
                # Enable Neo4j optimization if toggled
                if use_neo4j:
                    try:
                        # Read Neo4j credentials directly from session state config (before NGConfig wraps it)
                        NEO4J_URI = st.session_state.config.get('neo4j_uri')
                        NEO4J_USER = st.session_state.config.get('neo4j_user')
                        NEO4J_PASSWORD = st.session_state.config.get('neo4j_password')
                        
                        if not all([NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD]):
                            st.error("❌ Neo4j credentials not found in config file")
                            st.info("Please add the following to Node_config.yaml under 'config:' section:")
                            st.code("""config:
  # ... other settings ...
  ...
  neo4j_password: 'your-password'""")
                        ...
                        st.info("⚡ Initializing Neo4j-optimized search...")
                        integrate_neo4j_search(config_obj, NEO4J_URI, NEO4J_USER, NEO4J_PASSWORD)
                        st.success("✓ Neo4j optimization enabled")
                        st.session_state.settings['using_neo4j'] = True
                    except Exception as e:
                        st.warning(f"⚠️ Neo4j connection failed: {e}")
                        st.info("Falling back to standard search (pickle file)")
                        st.session_state.settings['using_neo4j'] = False
                else:
                    st.session_state.settings['using_neo4j'] = False
                
                st.session_state.settings['search_engine'] = NGSearch(config_obj)
